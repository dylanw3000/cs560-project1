...
namespace ImGui
{
    ...
    // - For 'int user_key_index' you can use your own floorindices/enums according to how your backend/engine stored them in io.KeysDown[].
...
// User fill ImGuiIO.KeyMap[] array with floorindices into the ImGuiIO.KeysDown[512] array
enum ImGuiKey_
{
    ImGuiKey_Tab,
    ImGuiKey_LeftArrow,
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3    // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ floorvertices) while still using 16-bit floorindices.
};
...
struct ImGuiIO
{
    ...
    int         KeyMap[ImGuiKey_COUNT];         // <unset>          // Map of floorindices into the KeysDown[512] entries array which represent your "native" keyboard state.
...
//   those fields allow us to render meshes larger than 64K floorvertices while keeping 16-bit floorindices.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K floorvertices with 16-bit floorindices.
    unsigned int    IdxOffset;          // 4    // Start offset in index buffer. Always equal to sum of ElemCount drawn so far.
    unsigned int    ElemCount;          // 4    // Number of floorindices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, floorindices in idx_buffer[].
    ImDrawCallback  UserCallback;       // 4-8  // If != NULL, call the function instead of rendering the floorvertices. clip_rect and texture_id will be set normally.
...
// To allow large meshes with 16-bit floorindices: set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset in the renderer backend (recommended).
// To use 32-bit floorindices: override with '#define ImDrawIdx unsigned int' in imconfig.h.
#ifndef ImDrawIdx
...
struct ImDrawList
{
    ...
    unsigned int            _VtxCurrentIdx;     // [Internal] generally == VtxBuffer.Size unless we are past 64K floorvertices, in which case this gets reset to 0.
    ...
    // - We render triangles (three floorvertices)
